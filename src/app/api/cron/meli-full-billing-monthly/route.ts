...
export async function GET(request: Request) {
  ...
  const period = normalizePeriodOverride(url.searchParams.get("period")) ?? getPreviousMonthPeriod();
    ...
    for (const { organizationId, store } of stores) {
      ...
      for (const account of accounts) {
        ...
        const syncRun = await startSyncRun({
          ...
          jobType: "meli-full-billing-monthly",
          details: {
            alias: account.alias,
        ...
        try {
          ...
          result.error =
            error instanceof Error
              ...
              : "Unknown Full billing sync error";
          await finishSyncRun({
            id: syncRun.id,
            status: "failed",
            startedAt: runStartedAt,
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error && error.message.includes("exceeded the data transfer quota")
            ...
            : error instanceof Error
              ? error.message
              : "Unknown Full billing cron error",
      },
      { status: 200 },
    );
  }
